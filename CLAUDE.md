...
- [`docs/standards/iteration_lessons.md`](docs/standards/iteration_lessons.md) — 17 durable rules promoted from sprint retros (Sprints 1–15; Sprints 16–17 have 2 lesson candidates each pending promotion at next confirmed-recurrence).
- [`docs/sync/`](docs/sync/) — `decisions-needed.md` (open: empty) + `ideas.md` (8 entries: 3 perf-architecture + 4 from mdata 0.8.3 feedback + 1 signal_eod write-lock concern from Sprint 17) + `mdata_chili_2026-05-14_0.8.5_delivery.md` (0.8.5 handoff; Sprint 17 wheel; closes wishlist v1) + `upstream_handle_lock_proposal_2026-05-14.md` (upstream author proposal re: handle write-lock contention). Earlier delivery docs (0.8.1/0.8.2/0.8.3/0.8.4) and `load_par_df_state_audit` moved to `docs/history/sync/`.
