...
- Test count on claude-2 (post-2026-05-13 main merge): **170 Rust** (`cargo test --workspace --exclude chili-py`; +4 from main's `crates/chili-op/tests/error_trace_test.rs`) + **72 chili-py pytest passing + 0 xfailed** (`uv run pytest`; +7 from Sprint 15 `TestParquetWriteConfig`).
