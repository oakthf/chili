...
- mdata delivery: **chili-sauce 0.8.6 wheel** at `dist/chili_sauce-0.8.6-cp310-abi3-macosx_11_0_arm64.whl` (Sprint 18 — `roll_tick` atomic tplog segment-rollover; mdata wishlist v2 P0) + handoff doc `docs/sync/mdata_chili_2026-05-16_0.8.6_delivery.md` + cross-comms reply draft `.cross_comms/drafts/chili_roll_tick_delivered_reply.json` (user-confirm before send). 0.8.5 wheel retained at `dist/` for reference; the 0.8.5 delivery doc moved to `docs/history/sync/`; all earlier delivery docs (0.8.1–0.8.4) already in `docs/history/sync/`. **⚠ Two distinct 0.8.6 builds exist (Sprint-19, 2026-05-16).** mdata's pinned/delivered wheel = the **Sprint-18** build, sha `8881337155b851b4ed4c1858eebaaa2a269b5ead176a92d13aea74cde3086ede` (`roll_tick` only; the delivery doc records this — frozen historical record, unchanged; mdata keeps this wheel). The on-disk `dist/chili_sauce-0.8.6-…whl` was **rebuilt from HEAD post-Sprint-19** (user decision: keep the `0.8.6` label, make artifacts current) → sha **`67a208b448c8bc82ca23723979fc2ecddcff524d3764493482d2ec44bf387fe2`** (Sprint 18 **+ 19**: roll_tick + IPC-remote-query + chiz). CLI `target/release/chili` likewise rebuilt from HEAD (`chili 0.8.6`). Both artifacts gitignored. The two 0.8.6 binaries differ by Sprint-19 content — residual cost of holding the label at 0.8.6; any future wheel carrying new content must bump (≥0.8.7) per the version-monotonicity principle.
