...
- [`docs/standards/iteration_lessons.md`](docs/standards/iteration_lessons.md) — 18 durable rules promoted from sprint retros (Sprints 1–15 + 1 from Sprint 18: verify-before-claim binds brief *mechanism* claims; Sprints 16–17 retain 2 candidates each pending confirmed-recurrence).
...
- Date pin: 2026-05-16.
- Versions on claude-2: chili-py at **0.8.6** (Sprint 18 wheel — adds `engine.roll_tick(log_dir, segment_label)` atomic tplog segment-rollover; sha256 in `docs/sync/mdata_chili_2026-05-16_0.8.6_delivery.md`; replaces 0.8.5 for mdata; 0.8.5 sha was `62e809129827d9f2514e5f5cbb506161f1281f1e7a4e3abd1a9e56f67efb5bf2`); workspace at **0.8.6** (Sprint-19 amendment 2026-05-16: aligned to claude-2's deliverable line — a strict superset must be ≥ upstream `0.8.2`, superseding MC-4's keep-0.8.1 inversion; whole workspace + chili-py wheel now coherently 0.8.6. claude-2 current with upstream `main` through `606d1cc` via Sprint-19 merge `f04e9e8`; prior sync 2026-05-13 commits 7ebb919..b91680f); Python polars pinned to `1.39.3`; Rust polars patched to `pola-rs/polars` at **`py-1.39.3` tag** + chili-side q-style fmt patch (currently a local clone at `/tmp/polars-py-1.39.3` — see user-driven backlog P0 below). The 0.8.1 wheel is BROKEN (segfaults when pyarrow is co-loaded) and must not be shipped.
...
- Test count on claude-2 (post-Sprint-19): **194 Rust** (`cargo test --workspace --exclude chili-py`; +12 Sprint 19: `chi_pkg_test.rs` 11 adopted from upstream `5a6adc5` + `fn_call_i64_test.rs` 1 ADD-1) + **96 chili-py pytest passing + 0 xfailed** (`uv run pytest`; +2 Sprint 19 `test_ipc_remote_query.py` ADD-2). Sprint 16/17/18 suites all green post-merge (zero regression).
...
- mdata delivery: **chili-sauce 0.8.6 wheel** at `dist/chili_sauce-0.8.6-cp310-abi3-macosx_11_0_arm64.whl` (Sprint 18 — `roll_tick` atomic tplog segment-rollover; mdata wishlist v2 P0) + handoff doc `docs/sync/mdata_chili_2026-05-16_0.8.6_delivery.md` + cross-comms reply draft `.cross_comms/drafts/chili_roll_tick_delivered_reply.json` (user-confirm before send). 0.8.5 wheel retained at `dist/` for reference; the 0.8.5 delivery doc moved to `docs/history/sync/`; all earlier delivery docs (0.8.1–0.8.4) already in `docs/history/sync/`. **⚠ Multiple distinct 0.8.6 dev builds exist — "0.8.6" label is frozen for delivery; the next *delivered* version is 0.8.7.** **mdata runs the IPC-superset lineage** (Sprint-18 `roll_tick` + Sprint-19 IPC-remote-query + chiz), **NOT** a roll_tick-only build — corrected 2026-05-18 per mdata; mdata is authoritative on its running wheel (cites sha `8878907…`). chili does **not** assert a sha for mdata's wheel. The chili-side local `dist/chili_sauce-0.8.6-…whl` shas (`8881337…` = Sprint-18-only dev build; `67a208b4…` = Sprint-18+19 HEAD rebuild) are **local build artifacts only, not the delivery base** — do not anchor delivery reasoning to them. **0.8.7 is cut from `claude-2` HEAD post-Sprint-21**: linear, `git merge-base --is-ancestor 606d1cc HEAD`=true, so a strict superset of roll_tick + IPC-remote-query + chiz + Sprint-20 + Sprint-21 — no structural regression path (cuts the tip, not an ancestor). Any future delivered wheel with new content bumps version per version-monotonicity; the delivery-base model is anchored to the **claude-2 HEAD IPC-superset lineage**, not a frozen sha.
...
- Next: **Sprint 20 EXECUTED — `main`→`claude-2` merge done, full pre-commit gate GREEN (fmt+clippy+`cargo test --workspace --exclude chili-py`+`maturin develop`+`pytest` 90 passed/0 failed), AWAITING USER RATIFICATION.** Adopted upstream lean refactors (rotate_handle, query_plan/add_at_time/GIL, lean wpar) + main's test/bench files; **preserved** the mdata superset: `publish_via_handle`, `roll_tick`/`init_tick(date)`, GR4 quant read-path helpers (M-1: eager `eval()` no longer auto-dequants — callers do; true-lazy ADR-0002/0003 kept), full-family `py-1.39.3` polars pin (decision #4). Dropped: `overwrite_partition` alias (2d), `compression`/`row_group_size` wpar kwargs (#3 → ADR-0005 superseded). 5 auto-merge-cascade bugs found+fixed (Cargo.toml dup keys ×2, io/util/bench chain, partition_filter 8-arg, `built_in_fn.rs` 9-arg wpar) — see iteration_lessons Lesson 19. Post-ratification: status flip + brief→`docs/history/sprints/` + the full project-state/test-count rewrite (deferred per Sprint-18/19 precedent). claude-2 current with upstream `main` through `main` tip (Sprint-19 base `606d1cc` + 3 commits `9dfa4d2`/`43faf44`/`ef4bfb2`).** mdata wishlist v2 P0 (`roll_tick`) closed Sprint 18 (ratified); mdata adopts in v1-25.2 (drops the planned Python drain barrier) and may submit further v2 items. New upstream surface available on claude-2: `engine.open_handle()`/`engine.sync()` (IPC remote query — note claude-2-adapted via `fn_call`), `import "@scope/pkg/mod"` (chiz `$CHIZPATH` package imports). 7+ ideas in `docs/sync/ideas.md` all gated on external triggers. **User-driven backlog** (still open, no timeline): (P0) GitHub-host the polars fork — replace `path = "/tmp/polars-py-1.39.3"` with `git = "..." + tag = "..."` in workspace + chili-py `[patch.crates-io]` blocks; without it, fresh clones break at `cargo build`. Until done, `vendor/polars-core/README.md` is the recovery protocol. (P1) KDB-X CE comparison once GA + interactive registration available. (P3) Box::new mitigation + A.2.2 vars-write-lock + P3.4 Categorical cache: all **deferred indefinitely** (Sprint 13/13.5 retros; no measurable targets).
