...
- Test count on claude-2 (post-Sprint-20): **198 Rust** (`cargo test --workspace --exclude chili-py`; Sprint 20: +`rotate_handle_test.rs` from main, net 194→198) + **90 chili-py pytest passing + 0 xfailed** (`uv run pytest`; Sprint 20: 96 −8 dead [`TestParquetWriteConfig`/`TestOverwritePartition` removed with #3/2d] +2 `TestM1EagerNoAutoDequant` M-1 guard). Full gate green post-merge (zero regression to preserved surfaces).
...
- Next: **Sprint 21 — mdata push-model D-1/D-2/D-3** (the 2026-05-17 proposal; mdata gates Q1–Q5 locked: Q1 Path-1 per-handle delivery cursor / Q2 message-shape interception (a) / Q3 raw-as-sent / Q4 per-drain-ack durability / Q5 no-fork+close-on-exec). D-1 event-driven fd+drain ≈ 1.5–2× roll_tick + D-3 resumable cursor as one surface; D-2 lazy accessor small parallel; D-4 deferred. Open design lock for the brief (audit#2 C6): `UpdEvent.seq_lo/seq_hi` field semantics. 0.8.7 cut from claude-2 HEAD after this sprint (IPC-superset by construction) → single mdata delivery. mdata still owes the explicit **(ii)+0.8.7-only** nod (gates the 0.8.7 *delivery*, not the sprints). mdata wishlist v2 P0 (`roll_tick`) closed Sprint 18; mdata adopts in v1-25.2. New upstream surface available on claude-2: `engine.open_handle()`/`engine.sync()` (IPC remote query — note claude-2-adapted via `fn_call`), `import "@scope/pkg/mod"` (chiz `$CHIZPATH` package imports). 7+ ideas in `docs/sync/ideas.md` all gated on external triggers. **User-driven backlog** (still open, no timeline): (P0) GitHub-host the polars fork — replace `path = "/tmp/polars-py-1.39.3"` with `git = "..." + tag = "..."` in workspace + chili-py `[patch.crates-io]` blocks; without it, fresh clones break at `cargo build`. Until done, `vendor/polars-core/README.md` is the recovery protocol. (P1) KDB-X CE comparison once GA + interactive registration available. (P3) Box::new mitigation + A.2.2 vars-write-lock + P3.4 Categorical cache: all **deferred indefinitely** (Sprint 13/13.5 retros; no measurable targets).
