...
- [`docs/sync/`](docs/sync/) — `decisions-needed.md` (open: empty) + `ideas.md` (3 architecture ideas, unscoped) + `mdata_chili_2026-05-09_delivery.md` (current 0.8.3 handoff doc) + `load_par_df_state_audit.md` (GIL-release safety audit, GREEN). Earlier delivery docs (0.8.1/0.8.2) moved to `docs/history/sync/`.
...
- Date pin: 2026-05-09.
- Versions on claude-2: chili-py at **0.8.3** (Sprint 15 wheel — bundles Sprint 14 GIL release on direct-FFI `load_par_df`/`clear_par_df` + Sprint 15 new `compression` / `row_group_size` kwargs on `write_partitioned_df` / `overwrite_partition`; default behavior byte-equivalent to 0.8.2 per ADR 0005 + sha256 regression check; shipped as `chili-sauce` distribution, import `chili`); workspace at 0.8.0; Python polars pinned to `1.39.3`; Rust polars patched to `pola-rs/polars` at **`py-1.39.3` tag** + chili-side q-style fmt patch (currently a local clone at `/tmp/polars-py-1.39.3` — see user-driven backlog P0 below). The 0.8.1 wheel is BROKEN (segfaults when pyarrow is co-loaded) and must not be shipped.
...
- Next: **Sprint 16+ scope unscoped** — pending mdata feedback on 0.8.3 + future-perf ideas in `docs/sync/ideas.md` (per-table mutex on par_df, RCU on par_df, coalesce concurrent loads on same hdb_path) trigger requires profile evidence on real workloads. Struct-shaped FFI for Parquet write options (ADR 0005 §6 — stop-using-positional-optional-args refactor) is the next API-shape sprint when ≥3 Parquet write options are needed. **User-driven backlog**: (P0) GitHub-host the polars fork — replace `path = "/tmp/polars-py-1.39.3"` with `git = "..." + tag = "..."` in workspace + chili-py `[patch.crates-io]` blocks; without it, fresh clones of chili break at `cargo build`. (P1) KDB-X CE comparison once GA + interactive registration available. (P2) mdata sign-off on 0.8.3 delivery. (P3) Sprint 13 P2 Box::new mitigation: **deferred indefinitely** per Sprint 13 lesson 2 (no measurable target — 17.7 % chili-side Box::new is bounded above by polars-internal `0x450c` 93.1 % polars-worker time which dominates the regression). A.2.2 vars-write-lock release **descoped indefinitely** per Sprint 13.5 retro (clone-then-swap memory cost vs unmeasured concurrent gain). P3.4 Categorical mapping cache **deferred indefinitely** per Sprint 13.5 categorical_eval bench (0.4 % Δ between repeated and distinct shapes; no measurable target).
