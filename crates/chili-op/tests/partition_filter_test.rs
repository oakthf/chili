...
use chili_op::{BUILT_IN_FN, write_partition_py};
use polars::prelude::*;
...
fn equality_returns_matching_partition_for_every_date() {
    ...
    for (date_str, day) in &[
        ...
        let df = eval_query(&engine, &format!("select from ohlcv where date={}", date_str));
        assert_eq!(
            df.height(),
            2,
...
fn lower_bound_only() {
    ...
    assert_eq!(df.height(), 4, "2 dates (01-05, 01-08) × 2 symbols = 4 rows");
}
...
fn non_partition_clause_before_partition_clause_does_not_drop_filter() {
    ...
    assert_eq!(df.height(), 1, "should scan 1 partition and keep 1 symbol row");
}
