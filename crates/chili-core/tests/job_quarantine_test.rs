//! FR-3 — scheduled-job quarantine-on-error (TorQ timer `runandreschedule`).
//!
//! A `.job` whose fire returns an error normally logs and KEEPS firing every
//! interval forever (the 2026-06-18 silent-repeated-failure shape). With
//! quarantine enabled (`set_jobs_deactivate_on_error(true)`) the engine
//! DEACTIVATES a failing job after its first error. These drive `execute_jobs`
//! directly (no scheduler thread / sleeps) for determinism.
