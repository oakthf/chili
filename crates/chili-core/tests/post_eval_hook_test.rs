//! FR-2b — post-eval audit hook (`fire_post_eval_hook`, TorQ `logusage.q`
//! `logAfter`/`logError`).
//!
//! The hook is the symmetric counterpart to the pre-eval hook: it fires AFTER
//! every inbound IPC request with `(user; handle; query; result; error)` for
//! its SIDE EFFECTS (audit logging). `result` is the evaluated value (Null on
//! error); `error` is the error string symbol (Null on success). The hook's
//! return value is IGNORED; an error it raises is swallowed (audit logging must
//! never break the request path). With no hook registered the eval path is
//! unchanged (zero overhead).
//!
//! These exercise the engine seam directly via `eval_with_pre_hook` (which
//! fires the post hook); no networking — the conn handlers call the same method.
...
/// Install a post-eval hook that records each fire's `(query; result; error)`
/// into engine vars `.audit.q` / `.audit.r` / `.audit.e`, plus a call counter
/// `.audit.n`, so the test can read them back after an eval. The hook returns
/// Null (its return is ignored anyway).
...
fn post_hook_fires_with_result_on_success() {
    let state = new_engine();
    // Define the queried var BEFORE installing the hook so the hook only counts
    // the request eval, not the setup.
    {
        let mut s = Stack::new(None, 0, 0, "");
        state
    ...
    // The hook fired exactly once with the result and a Null error.
...
fn post_hook_fires_with_error_on_failure() {
    ...
    let mut s = Stack::new(None, 0, 7, "alice");
    // A lookup of an undefined var errors; the error must propagate to the caller
    // AND the post-eval hook must still fire with the error string.
    ...
    assert_eq!(read_var(&state, ".audit.n").to_i64().unwrap(), 1);
    // result is Null on error; error carries a non-empty string.
    assert!(
...
fn clear_hook_stops_it_firing() {
    ...
    // The counter must still be 0 (the hook was cleared before any request).
...
fn hook_error_is_swallowed_not_propagated() {
    let state = new_engine();
    {
        ...
        state
            ...
            .unwrap();
        // A hook that always raises — its error MUST NOT break the request.
        state
            .eval(
                &mut s,
...
fn missing_hook_fn_is_swallowed() {
    ...
    // Register a name that is not defined — must be skipped, not error the request.
