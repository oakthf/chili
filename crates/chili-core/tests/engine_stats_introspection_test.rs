//! FR-B (v1-63) — engine introspection `stats()` tests.
//!
//! Verifies the new `.Q.w`/`.z.W` analog fields are present + sane:
//!   - `process_memory_rss_bytes`  (process RSS via sysinfo, > 0 in-process)
//!   - `queue_depth_total`         (sum of per-handle outbound queue depths)
//!   - `queue_depth_by_handle`     (per-handle series; the `.z.W` analog)
//!   - `handle_count` / `handle_nums`
//!   - `vars_len` / `topic_count`  (symbol-table width)
...
fn stats_exposes_new_introspection_fields() {
    ...
    // All FR-B fields must be present.
    for key in [
        ...
        assert!(s.contains_key(key), "stats() missing FR-B field '{}'", key);
    }

    // The pre-existing fields are still there (no regression).
...
fn stats_rss_is_positive_in_process() {
    ...
    match s.get("process_memory_rss_bytes") {
        Some(SpicyObj::I64(bytes)) => {
            // Reading our OWN process should always succeed → > 0 bytes.
...
fn stats_queue_depth_zero_with_no_subscribers() {
    ...
    // With no handles, handle_count is 0 and the per-handle series is empty.
...
fn stats_vars_len_reflects_bound_variables() {
    ...
    // Bind a variable via pepper eval, then re-read.
