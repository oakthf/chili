...
/// Value-preserving timezone relabel for one Datetime series.
///
/// Re-wraps the same underlying `i64` physical timestamps with `target_tz`
/// (a LABEL-ONLY change — `replace_time_zone` semantics, NOT
/// `convert_time_zone`, so the instant is never shifted). Returns the series
/// renamed to its original name and tagged with `target_tz`.
fn relabel_datetime_tz(s: &Series, target_tz: Option<TimeZone>) -> Series {
    ...
    let tu = *tu;
    // to_physical_repr() exposes the raw i64 without shifting; re-wrap it.
...
/// Make `incoming` extend-compatible with `target` for `df.extend` / upsert.
///
/// For every column present in BOTH frames where target & incoming are both
/// `Datetime` with the SAME `time_unit` but a DIFFERENT `time_zone`, relabel
/// the incoming column's tz-tag to the target's (value-preserving; both
/// directions naive↔tagged). Polars' `extend` rejects mismatched tz tags even
/// though the physical i64 is identical; this coerces the tag so a tplog /
/// subscribe-replay whose `ingest_ts` carries a different tz-tag than the
/// consumer's seeded column replays cleanly — matching AquaQ TorQ's native
/// tz-tolerant replay.
///
/// When all tz-tags already match (the common hot path) the incoming frame is
/// returned unchanged (no clone of column data) and this is effectively a
/// no-op aside from the per-column dtype comparison.
pub fn coerce_extend_tz(target: &DataFrame, incoming: &DataFrame) -> DataFrame {
    // Fast path: any column needing a relabel?
...
#[cfg(test)]
mod tz_coerce_tests {
    //! Unit tests for `coerce_extend_tz` — the tz-tolerant replay relabel.
    //!
    //! Reproduces the v1-64 cross-version replay bug: a tplog persisted
    //! `ingest_ts` UTC-tagged (`Datetime("ns","UTC")`) replayed into a consumer
    //! that seeded the column tz-naive (`Datetime("ns", None)`). Stock `df.extend`
    //! rejects the mismatched tz tag even though the physical i64 is identical.
    //! The helper relabels the incoming tag (value-preserving) so extend succeeds.
    ...
    fn utc_incoming_relabelled_to_naive_target_and_extends() {
        ...
        .unwrap();
        // incoming carries UTC-tagged ingest_ts (the pre-D-15 tplog shape).
        ...
        // tag now matches the naive target — value-preserving relabel.
        assert_eq!(
            coerced.column("ingest_ts").unwrap().dtype(),
            &DataType::Datetime(TimeUnit::Nanoseconds, None)
        );

        // extend (the stock-upd op) must now succeed.
        let mut t = target.clone();
        t.extend(&coerced).expect("extend after tz relabel");
        assert_eq!(t.height(), 4);
        // physical i64 preserved (no instant shift).
        let phys = t
            .column("ingest_ts")
            .unwrap()
    ...
    fn different_time_unit_is_left_untouched() {
        // tu differs -> NOT our case; leave it for polars' own error/coercion.
        let height = 1usize;
