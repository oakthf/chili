...
- `.broker.validateSeq` / `.broker.validateSeqStrict` and `file://` handle open now accept whole-file gzip sequence logs (`.seq.gz` or gzip magic), so `init_tick` recovery works on compressed tplogs without gunzip-to-temp
