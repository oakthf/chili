...
| 19 | upstream merge main→claude-2 — IPC remote query (`606d1cc` `fn_call` I64 arm + `eval_call` import + `engine.py` `open_handle`/`sync`) + chiz package imports (`5a6adc5` `import_source_path`/`resolve_package_*` + `chi_pkg_test.rs`). Real `git merge` not cherry-pick (lesson 4). `engine_state.rs` auto-merged CLEAN (disjoint-surface premise held); conflicts mechanical (engine.py union, versions kept claude-2's [workspace amended post-wrap 0.8.1→0.8.6 per user — MC-4 keep-0.8.1 superseded as a content/version inversion], serde_json+serial_test, tempfile deduped, chili-py/Cargo.lock untracked, Cargo.lock regen). 3-agent pre-exec audit (MC-1 chili-core add/add; MC-2 accept upstream fn_call I64 empty-args panic verbatim; MC-3 chi_pkg_test gate-safe; MC-4 version scope; MC-5 REFUTED planner housekeeping-overdue [Sprint 18 0ada8d8 already swept]; +ADD-1/ADD-2). **ADD-2 caught a broken headline feature**: upstream `sync()` `self.eval("pyHandle",[query])` ↔ claude-2 ADR-0002-diverged `eval()` (src_path 2nd-positional) → TypeError; adapted to `fn_call` (606d1cc's own I64 arm). clippy-conformance on upstream `resolve_package_version` (ptr_arg + 7 collapsible_if → let-chains). merge `f04e9e8` + fixups `b0b5f89`. | 3–6 (mid 4.5, post-audit) | ~6 | +33% vs post-audit mid (upper edge) | 0 (disjoint-surface premise held; no pivot) | several (disk-full env block surfaced + recovered by user; "do brief+audit first" go; "see if ok now") | ~2 in-merge (clippy `-D` fail on upstream 5a6adc5 code; ADD-2 caught upstream `sync()` incompat — both fixed pre-commit) + 1 env (disk exhaustion mid maturin, user-recovered) | +14 (12 Rust: `chi_pkg_test` 11 adopted + `fn_call_i64_test` 1; 2 chili-py pytest `test_ipc_remote_query`) | 2026-05-16 (Sprint 19 wrap commit) |
...
| 20 | main→claude-2 merge — adopt hinmeru lean refactors (rotate_handle/query_plan/add_at_time/GIL, lean 7-arg wpar, 6 AA test/bench) + preserve mdata superset (publish_via_handle 2b, roll_tick/init_tick(date) 2c, GR4 helpers + **M-1** eager-eval-no-auto-dequant, true-lazy ADR-0002/0003, full-family py-1.39.3 pin #4); drop overwrite_partition (2d) + wpar compression/row_group_size kwargs (#3→ADR-0005 superseded). 16 conflicts per audited A.2; engine_state.rs `roll_tick`+`rotate_handle` both-keep w/ injected brace (compile-verified); engine.py/lib.rs --ours+surgery. **5 auto-merge cascades** (NOT in 16, missed by 2 audits): chili-bin/chili-op Cargo.toml dup keys, io.rs→util.rs→benches/common chain, partition_filter_test 8-arg, **built_in_fn.rs 9-arg wpar** (manifested as false "query_plan regression"). Brief's "2a take main's bare eval" corrected (would've regressed ADR-0003). M-1 committed guard `TestM1EagerNoAutoDequant` added. Step-0 surfaced+fixed the `-- fmt_test` false-green verifier bug. → Lesson 19 + 3 memories. | 7–16 (mid 11, post-audit#1+#2) | ~12–15 | +10–35% (upper band, as audit warned) | 0 plan-pivots (1 user-corrected over-checkpointing detour) | several (3 behavioral corrections → memories; "what's the reason stopping"; go-aheads) | 5 auto-merge cascades + 1 brief-instruction error (2a) + 1 false alarm dismissed (mimalloc) — all caught pre-commit by gate/reads | Rust: +`rotate_handle_test.rs`(main) −`TestParquetWriteConfig`/`TestOverwritePartition`; chili-py pytest 96→90 (−8 dead #3/2d, +2 M-1 guard) | 2026-05-18 (gate-green; awaiting ratification) |
| 21 | mdata push-model D-1/D-2/D-3 — **D-1** GIL-free outbound upd notification (new `chili-core/upd_notify.rs`: POSIX self-pipe [macOS no eventfd/pipe2 → pipe+fcntl, O_NONBLOCK+FD_CLOEXEC] + bounded crossbeam_channel(4096), blocking-never-drop; message-shape interception in `handle_chili_conn` post-async-eval on the still-borrowed `(`upd;table;data)` MixedList, zero pyo3 on path; cursor_lo/hi = per-handle tick_count snapshot before/after; PyO3 `UpdEvent` #[pyclass] + upd_notify_fd/drain_upds + engine.py). **D-3** resumable cursor (`EngineState.resume_cursor` map + `resume_start_for` min-over-topics [Q1 Path-1; mdata owns per-row dedup] + `resume_cursor` pepper builtin + subscribe(resume_from)/set_resume_cursors FFI + sub.pep `.sub.init`/`.sub.recover` off hardcoded 0/latent tick[0]). **D-2** get_var_lazy snapshot.lazy(). ADR-0006 finalized (crossbeam 0.8.4→**0.5.15** verify-before-claim; deps in chili-core not chili-py; committed cross-thread no-deadlock review). 1 regression caught+fixed by the committed pytest guards: `to_str_vec` errors on non-empty MixedList → `resume_cursor[topics]` broke the no-resume `.sub.init` (7 tests incl. 4 pre-existing); fixed with a path-local tolerant extractor (not the shared helper). impl `4c3fe0c` (kickoff `4017674`). | 14–22 (mid 18, post-audit) | ~16–18 (best-effort; spans /compact) | ≈ mid-band (below 22 — no design iteration; halt-trigger-2/2b never fired) | 0 plan-pivots | resume after /compact ("compact done. please continue"); no behavioral corrections this sprint | 1 regression (`to_str_vec`-non-empty-MixedList breaks no-resume subscribe) caught pre-commit by the committed D-3 pytest guard + fixed in one edit | Rust 198→**201** (+3 `upd_notify_test.rs`); pytest 90→**97** (+7 `test_push_model.py`) | 2026-05-19 (gate-green; awaiting ratification) |
| 22 | mdata wishlist 2026-05-23 — **W1** `eval_str` pepper builtin (eval.rs + SIDE_EFFECT_FN reg; accepts `Str \| Sym` because chili-py FFI converts Python str → SpicyObj::Symbol; raw return — contract delta vs evalc/evali) + `sync()` docstring rewrite documenting str/bytes/tuple polymorphism (the actual W1 ask per mdata's turn-9 self-discovery: `sync(h, b"1+1")==2` works in 0.8.7+). **W2** graceful bare-TCP-connect on `start_tcp_listener` — 14 `.unwrap()` + 1 `panic!` → match/info!/continue across accept loop AND validate_auth_token (lines 2495-2660). MC-13 latency: <1ms server-side overhead avg over 100 iters asserted in committed test. **W3** deferred with explicit re-evaluation gate (poll-on-variable workaround viable per turn-9). 3-agent pre-impl audit (Explore+code-reviewer+planner) found 6 priors → 13 MC folded; post-impl code-reviewer found CRITICAL turn-9 wishlist drift (mdata withdrew W1 mid-sprint); user chose Option C (ship eval_str + document both paths). 1 mid-impl correction: `ArgType::Str` → `StrOrSym` caught by the W1 closure-gate pytest (test_sync_eval_str_simple, the MC-4 mandatory E2E test). Polars-fork `/tmp/polars-py-1.39.3` recovery protocol exercised first-try (4s clone + patch apply). **0.8.8 wheel** at `dist/chili_sauce-0.8.8-cp310-abi3-macosx_11_0_arm64.whl` (sha256 `e75dffb7c5621d3cc8c206828f8db2a6ff5a43442c53ad37b181c7914f963ddc`). Delivery doc `docs/sync/mdata_chili_2026-05-23_0.8.8_delivery.md`. impl `00db7b7` (brief+audit `fb88a44`). | 7–13 (mid 10, post-audit) | ~9–11 (upper-mid band) | ~0% vs post-audit mid (within 8–11pp revised expected) | 0 plan-pivots | ratify kickoff + Option-C choice on W1 + audit-folding go-aheads (~3-4 touches) | 1 mid-impl gate defect (`ArgType::Str` rejects Symbol; closure-gate pytest caught it pre-commit) + 1 fmt-line-wrap touchup applied via `cargo fmt --all` | Rust 201→**210** (+6 `eval_str_test.rs` + 3 `tcp_listener_graceful_test.rs`); pytest 97→**100** (+3 `test_eval_str.py`) | 2026-05-23 (gate-green; awaiting ratification) |
| 23 | mdata W3 Python-callable bridge — `engine.register_fn(name, callable, arity)` + `engine.unregister_fn(name)` via new `ExternalFnDispatcher` trait + `Func::external_name: Option<String>` + W3 dispatch branch in `eval_fn_call`. Two pre-sprint hazards measured: (1) re-entrancy proved safe by code-trace (chili-core eval + chili-op grep return zero `vars.read/write` across function-dispatch boundaries), (2) GIL overhead 151ns per py.detach+with_gil cycle → ~300ns per W3 round-trip + Python body. **2-round audit:** Round 1 3-agent (Explore + code-reviewer + planner) found 13 MCs; Round 2 (planner + code-reviewer) found 7 fold-gap items after the operational MCs were folded into canonical brief sections. ADR-0007 drafted FIRST per MC-1 (pre-impl gate #0); `.cross_comms/outbox` API-contract notification to mdata pre-Part-A. impl `ae5668b` (Part A chili-core, 5 Rust unit tests) + `3dc282c` (Part B chili-py FFI, 8 pytest including remote chili-IPC closure gate + concurrent register/dispatch). **0.8.9 wheel** sha256 `6912e3823e540ae37e8a70a2773f7b961d76cce87746c043891037c515550780`. Delivery doc `docs/sync/mdata_chili_2026-05-24_0.8.9_delivery.md`. **GR5 bench gate**: dev mac noise floor (~25%) exceeded the ±2% gate sensitivity → matched-shell A/B methodology adopted (force-reinstall + run both wheels in same shell); 0.8.9 = 0.8.8 within ±3% on N=1/N=4 concurrent_eval; halt criterion #1 does NOT fire. Promotes Lesson L21 (matched-shell A/B beats snapshot-vs-snapshot when noise floor > gate sensitivity) and L22 (operational MCs must fold into canonical brief sections, not just the audit appendix). | 13–17 (mid 15, post-audit MC-10 from original 12–17 mid 14) | ~12–14 (lower-mid; audit pre-empted most discovery work) | ~–10% (under mid) | 0 plan-pivots; 1 false-alarm bench regression caught + resolved | mdata gate-clear recorded as D-001 (in-session verbal); ratify brief; ratify ADR-0007; choice of audit-MC fold methodology; choice of bench-gate methodology revision | 1 bench false-alarm (>50% throughput drop traced to system-load noise, not W3); 1 mid-impl `del_var` returns Ok contract surprise (caught + fixed by has_var probe per MC-13); 0 gate defects committed | Rust 210→**215** (+5 `external_fn_test.rs`); pytest 100→**108** (+8 `test_register_fn.py` — brief predicted +6, MC-13 added +2 unregister variants) | 2026-05-24 (gate-green; awaiting ratification) |
