...
| **10 — Symbol pushdown (WL 2.2)** | **SHIPPED 2026-04-12** ✓ | **Q1-Q12 rerun** after HDB rewrite with sorted partitions | **DONE 2026-04-12** — ohlcv_1d rewritten with ~16 row groups/file (direct Polars, bypassing wpar shard issue). CSV at `benchmarks/q1-q12_post_phase10.csv`. Chili Q1 3.8ms (-44%), Q2 185ms (-55%), Q7 197ms (-54%), Q8 12.6ms (-38%), Q10 11.9ms (-64%), Q11 862ms (-53% but still FAIL target). |
| **11 — Fork guard (WL 1.2)** | **SHIPPED 2026-04-12** ✓ PID check on load/eval/wpar; clear error message with "use spawn not fork" guidance | **validate error message** | **DONE 2026-04-12** — validated: parent-loaded engine in forked child raises `RuntimeError` with clear "use spawn not fork" guidance. No code change needed. |
| **12 — Lifecycle API (WL 3.1)** | **SHIPPED 2026-04-12** ✓ close/unload/reload/is_loaded/table_count — mdata can swap ChiliGateway._read_engine=None for engine.reload() | **swap `ChiliGateway.close()`** | **DONE 2026-04-12** — `write_partition` now calls `engine.reload()` instead of dropping the engine. `close()` calls `engine.close()`. Commit `58b607a`. |
| **13 — Structured errors (WL 3.3)** | **SHIPPED 2026-04-12** ✓ 7 Python exception types: ChiliError, PepperParseError, PepperEvalError, PartitionError, TypeMismatchError, NameError, SerializationError. All extend RuntimeError (backwards-compat). | **swap RuntimeError catches** for specific types | **DONE 2026-04-12** — debug logging of structured exception type added to `query()`. No existing `RuntimeError` catches to replace — errors propagate unchanged (backwards-compat maintained). Commit `58b607a`. |
| **14 — Observability (WL 3.2)** | **SHIPPED 2026-04-12** ✓ stats() + parse_cache_len() work. query_plan() stubbed — needs lazy-mode eval path. | **wire stats() into rest prometheus** | **DONE 2026-04-12** — `/metrics` REST endpoint added, exposing `engine.stats()`, `is_loaded()`, `table_count()`. Exempt from API key auth. `query_plan` deferred (stubbed in chili). Commit `58b607a`. |
| **15 — Quantized helper (WL 3.4)** | **SHIPPED 2026-04-12** ✓ set_column_scale(table, col, factor) + clear_column_scales(). Auto-dequantizes Int64→Float64 in eval() results. | **remove dequantize_ohlcv_lazy** from chili-backed path + validate | **DONE 2026-04-12** — `set_column_scale()` wired at engine load for all price columns. Python-side `dequantize_ohlcv` removed from chili query path. Volume UInt64→Float64 retained in Python (chili scale only handles Int64). `raw=True` uses clear/restore pattern. Commit `58b607a`. |
| **16 — Broker bindings (WL 1.1)** | **SHIPPED 2026-04-12** ✓ `Engine.publish(topic, ipc_bytes) -> int` + `Engine.subscribe(topics, callback)` + `Engine.tick_upd(table, df) -> int`. In-process pub/sub via bounded mpsc channels (1024 cap), background Rust threads with GIL-safe callback dispatch, per-topic monotonic seq, backpressure logging, AtomicBool shutdown flag. `broker_eod` stubbed (NotImplementedError — EOD shape not frozen). 11 pytest tests: round-trip, isolation, fan-out (2/4/8), seq monotonic, seq independence, stress (2k×4 subs), tick_upd, no-sub publish, eod stub. Also fixed pre-existing `_column_scales` class-variable bug (Phase 15). | **add ChiliBrokerPublisher/Subscriber** to `pubsub.py` + run parity test against chili backend | **DONE 2026-04-12** — `ChiliBrokerPublisher`/`ChiliBrokerSubscriber` added to `pubsub.py`. `RdbSubscriberCache` refactored to accept any `Subscriber`. Broker parity test: 14 passed, 4 xfailed (EOD + replay as expected). 9 new unit tests. `broker_enabled` config flag added. Commit `58b607a`. |
| **17 — Agg pushdown (WL 2.3)** | **OPTIONAL / WONTFIX for mdata** — Phase 17 profiling on real 252-partition HDB shows chili Q11 already at **181ms** (within the original 70–200ms target). Phases 10+15 closed the gap from 1,850ms without Rust-side agg pushdown. Remaining 181ms→63ms gap (vs Polars native) is a chili-side nice-to-have, not an mdata blocker. Profiling results at `benchmarks/phase17_profile_results.json`. | N/A — target met | **CLOSED 2026-04-12** — Q11 target (<200ms) satisfied. No further mdata deliverables needed. |
