...
**Status:** **Accepted — pending impl validation.** Pre-drafted at Sprint 23 gate #0 (per `~/.claude/rules/self-audit-on-plans.md` MC-1: lock the contract before impl; post-impl code-reviewer pass confirms no revisions needed). Impl-time corrections (if any) will land inline and be re-published to mdata before wheel cut.
