...
**Status:** **SUPERSEDED 2026-05-25 (Sprint 24 main-port).** mdata's Revision A (`docs/sync/mdata_architecture_handoff_2026-05-24.md`) reframed away from the push-model: subscribers use `sub.pep`'s `upd:{[t;d] t upsert d; ...}` automatic accumulation (runs in chili's Rust receive thread; no Python in the data path) + `get_var(table)` on demand. The drain-loop / `upd_notify_fd` / `drain_upds` / `UpdEvent` / `resume_cursor` Python-side surface was ~530 LOC of redundant orchestration. Deleted in commit `da6b1a4` (Sprint 24 merge); 0.9.0 wheel ships without push-model. ADR preserved as historical record. **Original status:** Accepted, implemented Sprint 21 (commit `4c3fe0c`), gate-green Rust 201 / pytest 97; mdata-found §4 resume-coordinate doc-bug corrected `a50cec9`.
