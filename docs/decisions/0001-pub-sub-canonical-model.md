...
- **Sprint 18 follow-up (2026-05-16):** `EngineState::roll_tick` (mdata wishlist v2 P0, thread `mdata-chili-eod-upd-race-2026-05-15`) is **cutover-only and deliberately does NOT touch `signal_eod`**. It operates exclusively on the `ConnType::Sequence` tplog handle (open-next → fsync → same-id swap → close-old under one `handle.write()`); `signal_eod` operates exclusively on `ConnType::Publishing` subscriber handles. The two are independent by construction — verified in the Sprint 18 post-impl review. The EOD broadcast (this ADR's canonical 1→N path) remains the explicit responsibility of `engine.eod(d)`; `roll_tick` only swaps the durable log writer. No semantic change to ADR 0001. See `docs/sync/mdata_chili_2026-05-16_0.8.6_delivery.md` + Sprint 18 retro.
