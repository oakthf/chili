...
- Per the chili user's standing decision (recorded in our Sprint-20 brief): **no post-merge 0.8.6 wheel will ever be delivered to mdata.** The next — and only — artifact mdata receives is **chili-py 0.8.7**, built from HEAD **after BOTH** (a) the Sprint-20 `main`→`claude-2` merge **and** (b) the push-model D-1/D-2/D-3 implementation land. mdata stays on its pinned **Sprint-18 0.8.6** wheel (sha `8881337…`) until 0.8.7.
