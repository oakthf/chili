...
**(ii) + 0.8.7-only nod — RECEIVED 2026-05-19, both YES, no caveats.** (a) Option (ii) accepted, (i) preference withdrawn — mdata's `write_partitioned_df` `compression=`/`row_group_size=` kwarg-drop is already landed (mdata commit `b8f279a`, gate green 769p), so a 0.8.7 install will not hit the signature break. (b) 0.8.7-only delivery accepted — no intermediate 0.8.6 wheel; mdata's installed 0.8.6 runs unchanged until the single 0.8.7 handover. Recorded mdata-side in `chili_wishlist_2026-05-17_push-model.md` (mdata commit `fc4c800`). On receipt mdata will sha-verify the wheel == `1608317e…`, bump the pin, then run the 769-suite + D-1/D-2/D-3 acceptance + the ADR-0005 row-group re-bench. **Nothing further gates the 0.8.7 delivery** (Sprint 21 ratified 2026-05-19; wheel built; delivery doc `docs/sync/mdata_chili_2026-05-19_0.8.7_delivery.md`).
