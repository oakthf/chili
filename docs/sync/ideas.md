...
- [architecture] **`signal_eod` + `publish` hold `handle.write()` across TCP writes** — both paths acquire the handle write-lock and hold it while iterating subscribers and writing IPC bytes. At scale-out with many subscribers the lock becomes a serialization point and a slow-subscriber blocks all others. Fix path: either release-and-reacquire per subscriber or snapshot the handle list under a brief read-lock and release before the write loop. Trigger: profile evidence of lock contention at N≥16 subscribers OR an upstream author response to `docs/sync/upstream_handle_lock_proposal_2026-05-14.md`. See that doc for the full upstream proposal text. (Surfaced Sprint 17 code-reviewer; pre-existing convention, not a Sprint 17 regression.)
